...
- [x] **Phase 12 — Selaraskan style ke design-system webtools**: import `src/styles/design-system.css` (token) + `tools.css` (font Plus Jakarta Sans/Geomini); `C`/`R`/shadow dipetakan ke `var(--*)` (aksen merah brand, radius/shadow/shared), font jadi Plus Jakarta Sans, hapus Google Fonts Inter. Wrapper jadi scroll-container (`100dvh` + `overflowY:auto`) karena `design-system.css` kunci `body{overflow:hidden}`.
